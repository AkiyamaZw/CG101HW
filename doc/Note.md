...
1. **算法1**:上述过程中算法1提供的空间结构划分方法是一种类似于“主成分二分法”。笼统地说，进行一次空间分割：获得所有图元三角形“中心”X，Y，Z范围，取取值范围大的轴，将其视为分割主成分，按2分的思想将其分为2个空间分割, 使得两个空间有相同数量的图元三角形。整体上，对完整空间做一次分割，然后对分割出来的2部分递归调用分割算法。
2. **算法2**: 递归算法，见虎书第12.3.2节介绍
3. **算法3**: 相对简单，见虎书第12.3.1节介绍
4. **算法4**:101课程里介绍的Moller Trumbore Alg.
