...
void Renderer::Render(const Scene &scene) {
  ...
  for (int j = 0; j < scene.height; ++j) {
    for (int i = 0; i < scene.width; ++i) {
      ...
      // both of them wit the variable *scale*, and x (horizontal) variable
      // with the *imageAspectRatio*
      float x = ((2 * i + 1.f - scene.width) / scene.width) * scale *
                imageAspectRatio;
      float y = scale - (j + .5f) * 2.0f * scale / scene.height;
  ...
  std::string save_path =
      std::filesystem::path(std::string(OUT_DIR)).concat("hw5_out.ppm");
  std::cout << "save to path " << save_path.c_str() << std::endl;
