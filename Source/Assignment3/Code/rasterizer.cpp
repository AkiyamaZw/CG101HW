...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos)
{
    ...
    auto v = t.toVector4();
    auto min_3 = [](float a, float b, float c) -> int{
        return floor(std::min(a, std::min(b, c)));
    };
    auto max_3 = [](float a, float b, float c)->int{
        return ceil(std::max(a, std::max(b, c)));
    };
    auto norm_uv = [](Eigen::Vector2f& uv){
        if(uv.x() > 1) uv[0] = 1;
        if(uv.x() < 0) uv[0] = 0;
        if(uv.y() > 1) uv[1] = 1;
        if(uv.y() < 0) uv[1] = 0;
    };
    int min_x = min_3(v[0][0], v[1][0], v[2][0]);
    int max_x = max_3(v[0][0], v[1][0], v[2][0]);
    int min_y = min_3(v[0][1], v[1][1], v[2][1]);
    int max_y = max_3(v[0][1], v[1][1], v[2][1]);

    auto update_depth_and_color = [this](int x, int y, float test_depth, const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos, bool update_color_with_depth_check=true){
